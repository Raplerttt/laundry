    ...
    <?php else: ?>
        <?php foreach ($transactions as $index => $transaction): ?>
            <tr>
                ...
                <td class="p-2 border-b border-gray-200 border-r border-gray-300">
    <?= htmlspecialchars($transaction['nama_paket'] ?? '') ?>
</td>
                ...
                <td class="p-2 border-b border-gray-200 space-x-1">
                    <?php if ($transaction['status_pemesanan'] === 'Menunggu Pembayaran'): ?>
                      <a href="pembayaran.php?order_id=<?= $transaction['id'] ?>" class="px-2 py-1 bg-green-200 text-green-800 rounded text-xs">Bayar Sekarang</a>
                        <a href="invoice.php?id=<?= $transaction['id'] ?>" class="px-2 py-1 bg-gray-200 rounded text-xs">Lihat Bukti Pesanan</a>

                    <?php elseif ($transaction['status_pemesanan'] === 'Selesai'): ?>
                      <a href="ulasan.php?id=<?= $transaction['paket_id'] ?>" class="px-2 py-1 bg-blue-200 text-blue-800 rounded text-xs">Beri Ulasan</a>
                    <?php else: ?>
